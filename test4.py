...
tempData = np.zeros((120, 160), dtype = np.uint8)               # 用于存储像素点温度
...
def DataDisplay():
    ...
    """
        if fire.isFire() == 1:
        if fire.srcAngle < 0:
            print("火源位于左侧" + str(-fire.srcAngle) + '度')
        elif fire.srcAngle > 0:
            print("火源位于右侧" + str(fire.srcAngle) + '度')
        else:
            print("火源位于正前方")
    """
...
#
# 求图像中某一像素点相对于图像最下方中点的角度
#
def GetTargetAngle(target_x, target_y):
    imgMid_x = 80       # 图像最下方中点的x坐标
    imgMid_y = 0        # 图像最下方中点的y坐标
    dx = 0
    dy = 0
    angle = 0
    PI = 3.141592

    if target_x > imgMid_x:                         # 目标点位于图像中点右侧
        dx = target_x - imgMid_x
        dy = target_y - imgMid_y
        if dy == 0:                                 # 位于图像最下方，特殊处理
            angle = 180
        else:
            angle = math.atan(dx / dy) * 180 / PI   # atan返回夹角大小（弧度表示），转化成角度
    elif target_x < imgMid_x:                       # 目标点位于图像中点左侧
        dx = target_x - imgMid_x
        dy = target_y - imgMid_y
        if dy == 0:                                 # 位于图像最下方，特殊处理
            angle = -180
...
def SearchFire(target_x, target_y):
    ...
    if isFirePoint(target_x, target_y) == 1:        # 目标点为火焰再进行下一步处理
        ...
        fire.srcAngle = GetTargetAngle(fire.src_x, fire.src_y)                           # 获取火源角度
